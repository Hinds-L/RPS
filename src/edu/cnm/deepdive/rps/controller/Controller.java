...
public class Controller {
  ...
  private static final int MAX_SLEEP_ITERATION = 10;
  private static final int MIX_THRESHOLD = 10;
  private static final int PAIRS_TO_MIX = 2;
  ...
  private void initialize() {
    ...
    speedSlider.setMax(MAX_SLEEP_ITERATION);
    reset(null);
    timer = new Timer();
  }
...
//TODO Investigate whether the thread should re-enable the buttons when it's done.
  ...
  private class Runner extends Thread {
    ...
    public void run() {
      while (running){
        int sleep = (int) (1 + MAX_SLEEP_ITERATION - speedSlider.getValue());
        synchronized (lock) {
